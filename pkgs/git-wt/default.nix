...
writeShellApplication {
  ...
  text = ''
    ...
    _pr_cache_dir() {
      echo "''${WT_PR_CACHE_DIR:-''${XDG_CACHE_HOME:-$HOME/.cache}/git-wt/pr}"
    }
    ...
    _pr_fetch_repo() {
      ...
      local ignore_pred="false"
      if [ "''${#ignore_checks[@]}" -gt 0 ]; then
        local in_list
        in_list="$(printf '%s\n' "''${ignore_checks[@]}" | jq -R . | paste -sd, -)"
        ignore_pred="((.name // .context) | IN($in_list))"
      fi
      ...
      jq_prog='def norm:
          if .status != null then
            (if .status == "COMPLETED" then
               (if (.conclusion // "") | IN("SUCCESS", "NEUTRAL", "SKIPPED") then "ok" else "fail" end)
             else "pending" end)
          else
        ...
        | {
            idx: (.key | ltrimstr("b")),
            pr: .value.nodes[0],
            cs: [ ((.value.nodes[0].commits.nodes[0].commit.statusCheckRollup.contexts.nodes) // [])[]
                  | select( ('"$ignore_pred"') | not )
                  | norm ]
          }
        | [ .idx,
    ...
    pr-sync() {
      # --ignore-check applies to the repo named by the preceding --repo (or, before
      # any --repo, to every repo). repo_ignores[i] holds repo i's checks, newline-joined.
      local repo_paths=() repo_ignores=() default_ignores=""
      local cur=-1
      while [ "$#" -gt 0 ]; do
        case "$1" in
          --repo)
          ...
          --ignore-check)
            ...
            ;;
          --config)
            # JSON file [{path, ignoreChecks:[...]}], e.g. an opnix-managed secret, so
            # private repo paths and check names stay out of version control.
            local cfg_file="''${2:-}"
            shift 2
            if [ -f "$cfg_file" ]; then
              local cfg_count cfg_n
              cfg_count="$(jq 'length' "$cfg_file" 2>/dev/null || echo 0)"
              cfg_count="''${cfg_count:-0}"
              for (( cfg_n = 0; cfg_n < cfg_count; cfg_n++ )); do
                repo_paths+=("$(jq -r ".[$cfg_n].path" "$cfg_file")")
                repo_ignores+=("$(jq -r ".[$cfg_n].ignoreChecks // [] | .[]" "$cfg_file")")
                cur=$(( ''${#repo_paths[@]} - 1 ))
              done
            fi
            ;;
      ...
      if [ "''${#repo_paths[@]}" -eq 0 ]; then
        if [ -n "''${WT_PR_REPOS:-}" ]; then
          while IFS= read -r repo_line; do
            [ -n "$repo_line" ] || continue
            repo_paths+=("$repo_line")
            repo_ignores+=("")
          done <<< "$WT_PR_REPOS"
        else
          repo_paths+=("$(git rev-parse --show-toplevel)")
          repo_ignores+=("")
        fi
      fi

      if [ -n "''${WT_PR_IGNORE_CHECKS:-}" ]; then
        default_ignores+="$WT_PR_IGNORE_CHECKS"$'\n'
      fi
      ...
      for i in "''${!repo_paths[@]}"; do
        ...
        while IFS= read -r check_line; do
          [ -n "$check_line" ] && ignores+=("$check_line")
        done <<< "''${default_ignores}''${repo_ignores[$i]}"
        _pr_fetch_repo "$repo" "''${ignores[@]}" || true
      done
    }
    ...
    ls() {
      ...
      local header
      header="$(printf "%-''${max_session}s  %-''${max_branch}s  %-4s" "NAME" "BRANCH" "TMUX")"
      [ "$max_pr" -gt 0 ] && header="$(printf "%s  %-''${max_pr}s" "$header" "PR")"
    ...
    help() {
      ...
      echo "                        (defaults to \$WT_PR_REPOS, then the current repo)."
      echo "                        --ignore-check excludes a status check by name when deciding"
      echo "                        BROKEN/CI; it applies to the preceding --repo (or to all repos"
      echo "                        if given before any --repo, as does \$WT_PR_IGNORE_CHECKS)."
      echo "                        --config reads more repos from a JSON file"
      echo "                        ([{\"path\":..., \"ignoreChecks\":[...]}]) so private paths/checks"
      echo "                        can stay out of version control. Cache lives in \$WT_PR_CACHE_DIR."
